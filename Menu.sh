#!/bin/bash
#Configuración general para automatizar procesos
while :
do
		echo "		***********************************************"		
		echo "		*         Menu Principal                      *"
		...
		echo "		8.Intalación Docker"
		echo -e "	9.Salir del menu\n"

	read -p "Digite la opcion a ejecutar: " opcion
	echo "---------------------------------"
	case "$opcion" in
		"1")
		...
		"2")
			...
			sudo fdisk -l
			echo ""
			read -p "Ingrese el disco elegido: " disco
			echo ""
			sudo fdisk /dev/$disco
			;;
		"3")
			echo "Opcion para cambiar IP servidor"
			echo ""
			...
			;;
		"4")
			echo "Modificando la tabla de host"
			sudo nano /etc/hosts
			echo ""
			echo "La tabla de hosts ha sido modificada exitosamente"
			echo ""
			cd
			cd /etc 
			cat hosts
		...
		"5")
		    echo "**************************************************************"
			echo " *     Agregando permisos de firewall                          *"
			echo " "
			while :
			do
				echo "**************************************************************"
				echo " "
				echo "Escoja la opcion que desee para cambiar"
				echo  "los permisos en su firewall"
				echo " "
				echo " "
				echo "1.Habilitar servicio"
				echo "2.Estado actual del firewall"
			...
			;;
		"6")
			echo "      opcion para editar DNS Server   "
			while :
			do
				echo "----------------------------------"
				...
				echo "3.Detener servico"
				echo "4.Estado del servicio"
				echo "5.Reiniciar servico"
				echo "6.Editar"
				echo "7.Salir"
				echo "----------------------------------"
		...
		;;
		"7")
			echo "     opcion para configurar proxy      "
			echo "---------------------------------------"
			...
			echo ""
			echo "Aviso: Copie la configuración del proxy al final del documento"
			sleep 10
			cd
			sudo nano .bashrc
			echo ""
			echo "El proxy ha sido cambiado correctamente"
			echo ""
		;;
		"8")
			echo "-----------------------------------------------------------------------------"
